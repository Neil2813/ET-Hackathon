...
class SupplyNode:
    ...
    node_type: Literal["supplier", "logistics", "customer", "crude_producer", "refinery", "spr_terminal"] = "crude_producer"
    ...
    crude_grade: str = "medium_sour"
    api_gravity: float = 31.0
    sulfur_pct: float = 1.8
    refinery_profile: list[str] = field(default_factory=list)
    compatible_refineries: list[str] = field(default_factory=list)
    throughput_mbd: float = 0.0
    ...
    def from_context_supplier(cls, tenant_id: str, raw: dict) -> "SupplyNode":
        ...
        node_type_raw = str(raw.get("node_type") or raw.get("node_role") or "crude_producer")
        if node_type_raw not in {"supplier", "crude_producer", "refinery", "spr_terminal"}:
            node_type_raw = "crude_producer"
        return cls(
            id=node_id,
            name=supplier_name or node_id,
            tenant_id=tenant_id,
            node_type=node_type_raw,  # type: ignore[arg-type]
            lat=lat,
            lng=lng,
            country=country,
            ...
            crude_grade=str(raw.get("crude_grade") or "medium_sour"),
            api_gravity=_coerce_float(raw.get("api_gravity", raw.get("API_Gravity")), 31.0),
            sulfur_pct=_coerce_float(raw.get("sulfur_pct"), 1.8),
            refinery_profile=list(raw.get("refinery_profile", [])) if isinstance(raw.get("refinery_profile"), list) else [],
            compatible_refineries=list(raw.get("compatible_refineries", [])) if isinstance(raw.get("compatible_refineries"), list) else [],
            throughput_mbd=_coerce_float(raw.get("throughput_mbd"), 0.0),
            import_share_pct=_coerce_float(raw.get("import_share_pct"), 0.0),
        )
    ...
    def from_dataset_row(cls, tenant_id: str, raw: dict) -> "SupplyNode":
        ...
        return cls(
            ...
            name=str(raw.get("name", "")),
            tenant_id=tenant_id,
            node_type=str(raw.get("node_type") or raw.get("node_role") or "crude_producer"),  # type: ignore[arg-type]
            lat=float(raw.get("lat", 0)),
            lng=float(raw.get("lng", 0)),
            country=str(raw.get("country", "")),
            tier=tier_num,
            ...
            product_category=str(raw.get("category") or raw.get("products") or "crude oil"),
            crude_grade=str(raw.get("crude_grade") or "medium_sour"),
            api_gravity=_coerce_float(raw.get("api_gravity", raw.get("API_Gravity")), 31.0),
            sulfur_pct=_coerce_float(raw.get("sulfur_pct"), 1.8),
            refinery_profile=list(raw.get("refinery_profile", [])) if isinstance(raw.get("refinery_profile"), list) else [],
...
class CustomerSupplyGraph:
    ...
    def to_gnn_graph(self):
        ...
        for node in self.nodes.values():
            gnn.add_node(GNNNode(
                ...
                node_role=node.node_type,
                crude_grade=node.crude_grade,
                api_gravity=node.api_gravity,
                sulfur_pct=node.sulfur_pct,
                refinery_profile=node.refinery_profile,
                compatible_refineries=node.compatible_refineries,
                throughput_mbd=node.throughput_mbd,
                import_share_pct=node.import_share_pct,
    ...
    def get_pre_qualified_suppliers(self) -> list[SupplyNode]:
        ...
        return [
            ...
            if n.is_pre_qualified and n.node_type in {"supplier", "crude_producer"}
        ]
    ...
    def get_supplier_dicts(self) -> list[dict[str, Any]]:
        ...
        return [n.to_dict() for n in self.nodes.values() if n.node_type in {"supplier", "crude_producer"}]
