...
class SupplierNode:
    ...
    node_role: str = "crude_producer"  # crude_producer | refinery | spr_terminal | logistics
...
def _criticality_weight(node: SupplierNode) -> float:
    ...
    grade_weight = 1.15 if node.node_role == "crude_producer" and node.import_share_pct >= 15 else 1.0
    refinery_weight = 1.2 if node.node_role == "refinery" and node.throughput_mbd >= 0.5 else 1.0
    return weights.get(node.criticality, 1.0) * grade_weight * refinery_weight
...
class SupplyChainGraph:
    ...
    def propagate_risk(
        ...
        for nid, score in scores.items():
            ...
            # Days to stockout / reserve stress horizon
            if severity_factor > 0:
                if node.node_role in {"crude_producer", "spr_terminal"}:
                    node.days_to_stockout = round(node.safety_stock_days * (1 - min(0.75, score * 0.55)), 1)
                elif node.node_role == "refinery":
                    utilization_pressure = max(0.2, node.throughput_mbd) / 1.4
                    node.days_to_stockout = round(
                        node.safety_stock_days / (1 + severity_factor * score * utilization_pressure),
                        1,
                    )
                else:
                    node.days_to_stockout = round(
...
def build_graph_from_context(context: dict) -> SupplyChainGraph:
    ...
    for s in context.get("suppliers", []):
        node = SupplierNode(
            ...
            node_role=str(s.get("node_role") or s.get("node_type") or "crude_producer"),
            crude_grade=str(s.get("crude_grade") or "medium_sour"),
            api_gravity=float(s.get("api_gravity", s.get("API_Gravity", 31.0))),
            sulfur_pct=float(s.get("sulfur_pct", 1.8)),
            refinery_profile=list(s.get("refinery_profile", [])) if isinstance(s.get("refinery_profile"), list) else [],
            compatible_refineries=list(s.get("compatible_refineries", [])) if isinstance(s.get("compatible_refineries"), list) else [],
            throughput_mbd=float(s.get("throughput_mbd", 0.0)),
            import_share_pct=float(s.get("import_share_pct", 0.0)),
