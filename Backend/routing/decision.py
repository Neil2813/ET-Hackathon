...
VALID_ROUTE_MODES = {"sea", "air", "land", "hybrid", "tanker_vlcc", "tanker_suezmax"}
...
def route_days(row: dict[str, Any]) -> float:
    ...
    if mode in {"sea", "hybrid", "tanker_vlcc", "tanker_suezmax"}:
...
def crude_route_decision(
    route_rows: list[dict[str, Any]],
    *,
    current_mode: str = "tanker_vlcc",
    supply_gap_mbd: float = 1.6,
) -> dict[str, Any]:
    decision = enrich_route_decision(
        route_rows,
        current_mode=current_mode,
        customer_exposure_usd=supply_gap_mbd * 1_000_000 * 85,
    )
    for option in decision.get("route_options", []):
        mode = str(option.get("mode") or "")
        option["crude_viable"] = mode.startswith("tanker_")
        option["excluded_reason"] = "" if option["crude_viable"] else "Crude cannot be moved at national scale by air or land fallback."
    viable_tankers = [o for o in decision.get("route_options", []) if o.get("viable") and o.get("crude_viable")]
    if not viable_tankers:
        decision.update(
            {
                "recommended_mode": "",
                "delivery_answer": "No tanker corridor is viable under the current constraints.",
                "customer_impact_answer": "SPR drawdown and demand management are required until replacement cargoes are secured.",
            }
        )
        return decision
    ranked = sorted(viable_tankers, key=lambda o: _float(o.get("decision_score"), 999))
    recommended = ranked[0]
    for option in decision.get("route_options", []):
        option["recommended"] = option is recommended
        if option is recommended:
            option["status_label"] = "Best tanker option"
    decision["recommended_mode"] = recommended["mode"]
    decision["next_best_mode"] = ranked[1]["mode"] if len(ranked) > 1 else recommended["mode"]
    decision["delivery_answer"] = (
        f"Use {recommended['mode'].replace('_', ' ').upper()} via {recommended.get('lane', 'maritime corridor')} "
        f"for crude cargo arrival in {recommended['transit_days']} days."
    )
    decision["next_best_route_answer"] = "Fallback remains another tanker class or deferred cargo allocation; air and land are excluded for crude."
    decision["cost_answer"] = f"Estimated tanker movement cost is ${_float(recommended.get('cost_usd')):,.0f}, including charter and route-risk premiums."
    decision["customer_impact_answer"] = "Domestic fuel and power-sector stress should be managed with SPR drawdown until the alternate cargo window opens."
    return decision
