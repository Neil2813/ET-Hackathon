      ...
      <section className="relative overflow-hidden rounded-3xl border border-border/60 bg-[radial-gradient(circle_at_top_left,rgba(59,130,246,0.18),transparent_30%),radial-gradient(circle_at_top_right,rgba(234,88,12,0.16),transparent_35%),linear-gradient(180deg,rgba(15,23,42,0.94),rgba(7,10,18,0.98))] p-6 shadow-2xl shadow-black/20">
        ...
        <div className="relative grid gap-6 xl:grid-cols-[1.7fr_1fr]">
          <div className="space-y-4">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-white">Hormuz Blockade Energy Resilience Lab</h1>
              <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-300">
                This dashboard-native scenario shows how Praecantator detects a maritime crude disruption, scores crude producer
                and refinery exposure, schedules SPR drawdown, ranks tanker alternatives, and closes the audit trail with explicit assumptions.
              </p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="mt-1 text-sm text-slate-300">{demoAssessment.spr_cover_days} days SPR cover modeled</div>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="mt-1 text-sm text-slate-300">Cape tanker route + SPR bridge schedule</div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-cyan-400/20 bg-slate-950/60 p-5 backdrop-blur">
            ...
            <div className="mt-4 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-200">
                ...
                reserve bridge {simulation.probeNode.stockoutDays.toFixed(1)} days · modeled worst-case risk {fmtINR(simulation.probeNode.riskUsd)}.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl border border-border/60 bg-surface-low p-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-semibold text-white">Crude producers, refineries, and SPR nodes</h2>
            </div>
            <div className="mt-5 space-y-3">
                ...
                <div key={supplier.supplier_id} className="rounded-2xl border border-border/60 bg-slate-950/40 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      {(supplier.crude_grade || supplier.refinery_profile?.length || supplier.throughput_mbd) && (
                        <div className="mt-2 text-xs leading-5 text-slate-500">
                          {supplier.crude_grade && `Grade ${supplier.crude_grade} · API ${supplier.api_gravity} · sulfur ${supplier.sulfur_pct}% `}
                          {supplier.refinery_profile?.length ? `Profile ${supplier.refinery_profile.join(", ")} ` : ""}
                          {supplier.throughput_mbd ? `Throughput ${supplier.throughput_mbd} mbd` : ""}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="mt-3 flex items-center justify-between text-sm text-slate-300">
                    ...
                    <span>{supplier.is_backup ? "Reserve bridge asset" : "Primary energy dependency"}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-5 space-y-3">
              {demoRoutes.map((route) => (
                <div key={`${route.mode}-${route.engine ?? "na"}`} className="rounded-2xl border border-border/60 bg-slate-950/40 p-4">
                  <div className="flex items-center justify-between">
                    <div className="text-base font-semibold capitalize text-white">{route.mode.replace("_", " ")}</div>
                  ...
                  <div className="mt-2 text-sm text-slate-300">
                    {route.recommended
                      ? `Cape diversion arrives in ${route.transit_days} days and requires SPR bridge draw until discharge.`
                      : `${route.lane} remains constrained by ${route.chokepoint}; ${route.restrictions?.[0] ?? "not selected"}.`}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="rounded-3xl border border-border/60 bg-surface-low p-5">
          ...
          <div className="mt-5 rounded-3xl border border-orange-500/20 bg-[linear-gradient(180deg,rgba(251,146,60,0.10),rgba(15,23,42,0.55))] p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-lg font-semibold text-white">Hormuz Blockade · Critical energy incident</div>
                <div className="mt-1 text-sm text-slate-300">
                  {demoAssessment.affected_suppliers.length} energy nodes affected · {fmtINR(demoAssessment.exposure_usd)} exposure · {demoAssessment.days_at_risk} day cargo window
                </div>
              </div>
            ...
            <div className="mt-5 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-200">
                Approve the decision-agent package: charter the Cape-routed VLCC, initiate SPR drawdown, and allocate compatible crude to Jamnagar and Paradip until replacement cargoes arrive.
              </p>
            </div>
          </div>
        </div>
