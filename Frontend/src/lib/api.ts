...
export interface WorkflowRoutesResponse {
  ...
  recommended_mode: "sea" | "air" | "land" | "hybrid" | "tanker_vlcc" | "tanker_suezmax";
  next_best_mode?: "sea" | "air" | "land" | "hybrid" | "tanker_vlcc" | "tanker_suezmax" | "";
...
export interface SPRRequest {
  national_consumption_mbd?: number;
  spr_cover_days?: number;
  initial_fill_pct?: number;
  supply_gap_mbd?: number;
  refinery_throughput_mbd?: number;
  demand_shed_limit_pct?: number;
  planning_horizon_days?: number;
  replenishment_eta_days?: number;
}
...
export interface SPRDay {
  ...
  refinery_throughput_mbd: number;
  spr_draw_mbd: number;
  demand_management_mbd: number;
  unmet_demand_mbd: number;
  spr_inventory_mmbbl: number;
  spr_cover_days_remaining: number;
  stress_index: number;
}

export interface SPRResponse {
  initial_inventory_mmbbl: number;
  exhaustion_day: number | null;
  peak_unmet_demand_mbd: number;
...
export const api = {
  ...
  routes: {
    ...
    sprOptimize: (payload: SPRRequest) =>
      request<SPRResponse>("/workflow/spr-optimize", {
        method: "POST",
        body: JSON.stringify(payload),
      }),
  },
